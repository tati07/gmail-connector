...
import com.google.code.com.sun.mail.iap.CommandFailedException;
...
import com.google.code.com.sun.mail.imap.protocol.Item;
import com.google.code.com.sun.mail.imap.protocol.MessageSet;
import com.google.code.com.sun.mail.imap.protocol.RFC822DATA;
...
import com.google.code.javax.mail.UIDFolder;
...
public class IMAPMessage extends MimeMessage {
    ...
    private long x_gm_msgid = -1;
    private long x_gm_thrid = -1;
    private String[] x_gm_labels = null;
	...
	class FetchProfileCondition implements Utility.Condition {
	    ...
	    private boolean needSize = false;
            private boolean needXGmMsgid = false;
            private boolean needXGmThrid = false;
            private boolean needXGmLabels = false;
		...
		if (fp.contains(IMAPFolder.FetchProfileItem.SIZE))
		    needSize = true;
                if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_MSGID))
                    needXGmMsgid = true;
                if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_THRID))
                    needXGmThrid = true;
                if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_LABELS))
                    needXGmLabels = true;
		...
		if (needSize && m.size == -1)		// no size
		    return true;
                if (needXGmMsgid && m.getGoogleMessageId() == -1)
                    return true;
                if (needXGmThrid && m.getGoogleMessageThreadId() == -1)
                    return true;
                if (needXGmLabels && m.getGoogleMessageLabels() == null)
                    return true;
	...
	if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_MSGID)) {
	    command.append(first ? "X-GM-MSGID" : " X-GM-MSGID");
	    first = false;
	}        
	if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_THRID)) {
	    command.append(first ? "X-GM-THRID" : " X-GM-THRID");
	    first = false;
	}        
	if (fp.contains(IMAPFolder.FetchProfileItem.X_GM_LABELS)) {
	    command.append(first ? "X-GM-LABELS" : " X-GM-LABELS");
	    first = false;
	}        
	if (fp.contains(IMAPFolder.FetchProfileItem.HEADERS)) {
	    allHeaders = true;
	...
	Utility.Condition condition = new FetchProfileCondition(fp);
        ...
        // Acquire the GmailFolder's MessageCacheLock.
		...
		for (int j = 0; j < count; j++) {
		    ...
		    else if (item instanceof X_GM_MSGID) {
			X_GM_MSGID xgmid = (X_GM_MSGID)item;
			msg.x_gm_msgid = xgmid.x_gm_msgid; // set x_gm_msgid
		    }
		    else if (item instanceof X_GM_THRID) {
			X_GM_THRID xgtid = (X_GM_THRID)item;
			msg.x_gm_thrid = xgtid.x_gm_thrid; // set x_gm_thrid
		    }
		    else if (item instanceof X_GM_LABELS) {
			X_GM_LABELS xglab = (X_GM_LABELS)item;
			msg.x_gm_labels = xglab.x_gm_labels; // set x_gm_labels
		    }
